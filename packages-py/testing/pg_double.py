...
TEST SUPPORT ONLY. Nothing in packages-py imports this outside tests.
...
`test_db_writer.py::test_migration_declares_every_column_the_writer_uses` guards
the one drift that would make these tests actively misleading: the schema below
silently diverging from the real migration.
"""
