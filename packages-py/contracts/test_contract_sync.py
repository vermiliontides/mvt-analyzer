...
The bug these tests exist for: `contracts/normalized-record.schema.json` is
documented as the source of truth, and both language mirrors carried a comment
asking contributors to change all three files in the same commit. The mirrors
were updated with `ileapp_record` and the canonical schema was not, so records
from the iLEAPP extractor constructed cleanly through Pydantic and Zod and were
then rejected by the JSON schema the adapters load. Nothing failed until a
record was validated against the canonical file, which no test did.

So: the round trip through the *canonical* file is asserted here, not the
agreement between the two mirrors (which agreed with each other the whole time
...
from contracts_adapter import SCHEMA_PATH, load_schema, source_types, validate
from normalized_record import NormalizedRecord, SourceType

REPO_ROOT = Path(__file__).resolve().parents[2]
SYNC_SCRIPT = REPO_ROOT / "scripts" / "sync_contracts.py"
ZOD_MIRROR = REPO_ROOT / "packages-ts" / "contracts" / "normalizedRecord.ts"
...
def test_canonical_schema_resolves_from_the_adapter():
    ...
    An extractor launched as a subprocess by the orchestrator does not run from
    the repo root.
    """
    assert SCHEMA_PATH.is_file()
    assert SCHEMA_PATH == REPO_ROOT / "contracts" / "normalized-record.schema.json"


def test_there_is_exactly_one_schema_file_in_the_repo():
    """A second copy is how the enum drifted in the first place.

    `packages-ts/contracts/` used to hold its own `normalized-record.schema.json`
    with a different enum, imported by nothing.
    """
    found = [
        path
    ...
    assert found == [REPO_ROOT / "contracts" / "normalized-record.schema.json"]
...
def test_there_is_exactly_one_extractor_contract_document():
    ...
    assert found == [REPO_ROOT / "contracts" / "EXTRACTOR_CONTRACT.md"]
...
def test_sync_contracts_detects_enum_drift(tmp_path, monkeypatch):
    ...
    for relative in (
        ...
        "packages-py/contracts/normalized_record.py",
        "packages-ts/contracts/normalizedRecord.ts",
    ...
    schema_path = sandbox / "contracts/normalized-record.schema.json"
    ...
    assert 'KEYCHAIN_ENTRY = "keychain_entry"' in (sandbox / "packages-py/contracts/normalized_record.py").read_text()
    assert '"keychain_entry",' in (sandbox / "packages-ts/contracts/normalizedRecord.ts").read_text()
...
def test_generated_blocks_carry_a_do_not_edit_marker():
    ...
    for path in (
        REPO_ROOT / "packages-py/contracts/normalized_record.py",
        ZOD_MIRROR,
    ):
